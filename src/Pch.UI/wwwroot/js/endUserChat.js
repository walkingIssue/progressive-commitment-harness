...
function circuitUnavailable() {
    ...
    return root()?.dataset.browserCircuitState === "browser_circuit_disconnected" ||
        !Boolean(window.Blazor) ||
        isReconnectBlocked(modal);
}
function scheduleHttpTransport(shouldRun, action) {
    window.setTimeout(() => {
        if (!shouldRun())
            return;
        if (!circuitUnavailable() && root()?.dataset.providerOutcome !== "planner_model_pending")
            return;
        showDisconnectedState();
        void action();
...
function setModelRole(role) {
    ...
    document.querySelector("[data-model-status-strip='end-user']")?.setAttribute("data-live-preflight-state", liveSelected ? "blocked_by_guard" : "deterministic_default");
    document.querySelector("[data-model-status-strip='end-user']")?.setAttribute("data-live-proposal-state", liveSelected ? "not_run" : "not_requested");
    document.querySelector("[data-model-status-strip='end-user']")?.setAttribute("data-harness-validation-state", "not_run");
    document.querySelector("[data-model-status-strip='end-user']")?.setAttribute("data-latest-turn-source", liveSelected ? "live_model_proposal_blocked" : "deterministic_fallback");
        ...
        if (selectedCandidateId) {
            ...
            document.querySelectorAll("[data-http-primitive-turn] [data-field-input]").forEach((field) => {
                ...
                if (field.dataset.dateRangePart === "start") {
                    const end = document.querySelector(`[data-http-primitive-turn] [data-field-input-end='${cssEscape(fieldId)}']`);
                    fieldValues[fieldId] = `${field.value} to ${end?.value ?? field.value}`;
                    return;
                }
                fieldValues[fieldId] = field.value;
            });
            document.querySelectorAll("[data-http-primitive-turn] [data-field-input-multi]:checked").forEach((field) => {
...
function applyPlanningApiResponse(response, transport) {
...
function formPrimitiveHtml(turn, primitive) {
...
function normalizeRenderer(rendererKey) {
...
function missingRendererHtml(field, rendererKey) {
...
function dateRangeValues(value) {
...
function primitiveMessageHtml(turn, primitive) {
...
function renderApiTaskRail(state, turn) {
    const rail = document.querySelector("[data-task-rail='trip']");
    if (!rail)
        return;
...
function renderDevelopmentStatusDock(response, transport) {
    const latestTrace = response.trace.at(-1);
    let dock = document.querySelector("[data-development-status-dock='trip']");
    if (!dock) {
        ...
        dock = document.querySelector("[data-development-status-dock='trip']");
    }
    if (!dock)
        return;
...
function timelineItem(id, mode, kind, state, title, summary, originTurnId, media, attrs = "") {
...
function ensurePlanningTimeline() {
    if (document.querySelector("[data-planning-timeline='trip']")) {
        return;
    }
    chatMain()?.insertAdjacentHTML("beforeend", `<section class="planning-timeline" data-planning-timeline="trip" data-planning-timeline-mode="day" data-raw-absence-state="verified" aria-labelledby="planning-timeline-title">
...
function sendPrompt() {
    ...
    appendTurn("turn-provider-role-status", "provider", "role-status", "applied", "Offline deterministic model role is active by explicit selection.", "model_role_status_ready");
...
function setAskTabVisible(visible) {
    document.querySelectorAll("[data-ask-action='open']").forEach((button) => {
...
function toggleDrawer(open) {
    ...
    window.requestAnimationFrame(() => document.querySelector("[data-prompt-entry='trip-drawer']")?.focus());
}
...
function setTimelineMode(mode) {
    const timeline = document.querySelector("[data-planning-timeline='trip']");
    const rail = document.querySelector("[data-planning-timeline-rail]");
    if (!timeline || !rail)
        return;
...
function closeDrawerAfterFocusLeaves(target) {
...
function closeDrawerOnOutsidePointer(target) {
    const drawer = document.querySelector("[data-ask-drawer-panel]");
    if (!drawer)
        return;
    if (target instanceof HTMLElement && (drawer.contains(target) || target.closest("[data-ask-action='open']")))
        return;
    toggleDrawer(false);
}
...
function handleChatInteraction(target) {
    ...
    if (modeButton?.dataset.timelineModeAction) {
        ...
        scheduleFallback(() => document.querySelector("[data-planning-timeline='trip']")?.dataset.planningTimelineMode !== mode, () => setTimelineMode(mode));
    }
    ...
    if (originTurnId) {
        ...
        scheduleFallback(() => root()?.dataset.focusedTurnId !== originTurnId, () => focusOriginTurn(originTurnId));
    }
}
