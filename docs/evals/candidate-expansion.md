...
Accepted eval rows derive persisted slot ids and categories from the trusted packet slot map. Provider results with unknown slot ids, duplicate slot ids, or category mismatches use the fixed `candidate_expansion_slot_mismatch` outcome and omit result slot metadata. Packet/result id mismatches use the fixed `candidate_expansion_packet_id_mismatch` outcome and omit result slot metadata. Exceptions use fixed error codes and do not echo raw exception messages.
