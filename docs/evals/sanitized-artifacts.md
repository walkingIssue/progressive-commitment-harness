...
Live model role runner artifacts follow the same policy: accepted rows may persist trusted packet ids, role enums, configured model ids, allowlisted output kinds, allowlisted mood enums, response length, and accepted provider metadata. Rejected rows use fixed row identifiers and omit raw prompts, provider payloads, raw completion JSON, raw arguments, credentials, context digests, exception text, and untrusted mood prose.
