...
Latest integrated code before this progress update: `50b586d69123b158885ac6aafe2c064ae39d3894 Close Ask drawer on focus loss`

Overall position: Stage 2/3 now have deterministic session traversal, approval-gated action intake, replayable trace events, a harness-owned external action decoder, a reusable runtime action application result, an authority-checked mission intake application, a validated provider-shaped mission proposal adapter, a prompt packet boundary with runtime-only raw prompt transfer, canonical itinerary candidate application state, a live-turn projection contract for end-user transcripts, and first edit-impact analysis for planning history changes. Stage 4/5 now have provider-local mission planner runtime handoff, mission-kind allowlisting, bounded mission memory projection into `StagePacket`, a guarded OpenAI/OpenRouter-compatible live mission planner client behind deterministic tests, provider-local candidate expansion for itinerary slots, mock hold preparation, live model-role runner guardrails, and provider repair-posture scaffolding. Stage 5/6 now have sanitized model-action, mission-planner, candidate-expansion, hold-preparation, evidence-export, shared eval artifact redaction checks, provider-local fidelity eval rows, and a first canonical fidelity ownership matrix. Stage 7 now has deterministic availability/quote preview boundaries across harness, provider, and UI without booking/payment side effects. Stage 8 now has a harness-owned itinerary slot compiler, slot-scoped candidate ownership, selected/deferred itinerary decisions, and projection counters. Stage 9 now has a deterministic end-to-end Stage Cockpit run plus a presentable end-user planner with separated Stage Cockpit route, image-backed option cards, selected-card echoes, compact planning timeline, bottom Ask affordance, and browser-click regression coverage. Stage 10 now has deterministic replay audit, cross-provider sanitized artifact policy, UI release-smoke/a11y markers, fidelity release-gate markers, and raw-absence smoke coverage. The next decisive gap is live truth: the end-user route is still deterministic by default, so Sprint 019 should attach real models through a guarded harness-owned conductor and provider preflight path.
...
| 017 | Stage 9/10 | Visual card media | Provider-neutral media provenance, provider media registry, local generated Japan media pack, image-backed option cards, selected-card echoes, and plan-trail media | done |
| 018 | Stage 3/9/10 | Progressive planning history | End-user route split from Stage Cockpit, compact planning timeline, click-to-source history, edit-impact analyzer, repair posture scaffolding, and live-failure report posture | done |
| 019 | Stage 3/4/5/9/10 | Model-attached end-user loop | Planned sprint for harness live session conductor, provider live preflight/model turn runner, and guarded `/trip` model-attached UI integration | planned |
...
Sprint 012 hardened the deterministic end-to-end trip run into a repeatable release smoke surface.

- `TripRunReplayAudit` replays vacation, business, funeral/downtime, family-support, blocked compiler, blocked candidate, pending confirmation, and missing-approval scenarios without mutating source sessions.
- Replay results expose sanitized case/scenario ids, snapshot/result codes, deterministic snapshot hashes, bounded evidence references, and bounded trace references rather than raw snapshots or provider inputs.
- Provider eval artifact tests now share `SanitizedEvalArtifactAssert` and cover rejected/error rows across model action, mission planner, candidate expansion, hold preparation, and evidence export.
- Provider eval docs now define persisted-safe fields, fixed outcome/error-code behavior, trusted packet-map derivation, and no paid-provider fallback for optional live paths.
- Stage Cockpit end-to-end release smoke now exposes a stable release summary with accepted/blocked/pending path counts, primary run marker, evidence export marker, raw-absence marker, and keyboard-accessible run controls.
- The browser smoke covers happy path, pending confirmation, provider candidate mismatch, wrong-slot candidate, missing approval, and raw-sentinel export paths.

## Sprint 012 Verification

- `npm run build:ui`: passed.
- `dotnet build src/Pch.UI/Pch.UI.csproj`: passed, 0 warnings, 0 errors.
- `dotnet test tests/Pch.UI.Tests/Pch.UI.Tests.csproj`: 38 tests passed.
- `dotnet test`: 242 tests passed across core, harness, providers, and UI.
- `dotnet build`: passed, 0 warnings, 0 errors.
